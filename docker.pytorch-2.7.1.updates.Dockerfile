# pytorch/pytorch has Python 3.10.13 with conda. Tensorboard may need to be installed with conda, but still may not work.
...
RUN conda install --yes matplotlib mlxtend numpy pandas requests scikit-learn
RUN conda install --yes tensorboard tqdm
RUN conda install --yes torch-tb-profiler torchinfo
